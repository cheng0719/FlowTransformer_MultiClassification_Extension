...
class FlowTransformer:
    ...
    def _load_preprocessed_dataset(self, dataset_name:str,
        ...
        # extract the unique attack label values in class column, and ensure that the benign label is excluded from the class column
        ...
        # create a new dataframe with the columns: __training, __y, __seq_interval(Optional), and the numerical and categorical columns
        new_df = None
        ...
        model_input_spec = ModelInputSpecification(new_features, len(numerical_columns), levels_per_categorical_feature, self.input_encoding.required_input_format, attack_class_type_list=attack_class_type_list, all_class_types_list=[])
    ...
    def evaluate(self, m:keras.Model, batch_size, early_stopping_patience:int=5, epochs:int=100, steps_per_epoch:int=128):
        ...
        attack_classes_indices_test = {}
        for attack_class in self.model_input_spec.attack_class_type_list:
            mask = self.y.astype('str') == str(attack_class)
            attack_classes_indices_test[attack_class] = np.argwhere(~train_mask & mask).reshape(-1)
        ...
        print(f"Attack classes in eval set:")
        for attack_class in self.model_input_spec.attack_class_type_list:
        ...
        print(f"\nPositive samples in eval set: {n_eval_P}")
        print(f"Negative samples in eval set: {n_eval_N}")

        # compute the number of each attack class in the evaluation set
        print(f"\nComputing and print the number of each attack class in the evaluation set...")
        eval_attack_class_array_list = {}
        for attack_class in self.model_input_spec.attack_class_type_list:
            eval_attack_class_array_list[attack_class] = y_mask[attack_classes_indices_test[attack_class]]
            print(f"Attack class {attack_class} samples in eval set: {np.count_nonzero(eval_attack_class_array_list[attack_class])}")
        ...
        def run_evaluation(epoch):
            ...
            # compute the correct and wrong predictions for each attack class and append them to the `epoch_results`
            for attack_class in self.model_input_spec.attack_class_type_list:
                class_indices_in_test = np.isin(indices_test, attack_classes_indices_test[attack_class])

                class_pred_P = pred_P[class_indices_in_test]
                class_pred_N = pred_N[class_indices_in_test]
                class_eval_y = y_mask[indices_test][class_indices_in_test]
                
                CORRECT = np.count_nonzero(class_pred_P & class_eval_y)
                WRONG = np.count_nonzero(class_pred_N & class_eval_y)
                print(f"{attack_class}: CORRECT = {CORRECT}, WRONG = {WRONG}")
